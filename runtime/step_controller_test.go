...
func (n *mockStepNode) Run(ctx context.Context, env *core.Envelope) (*core.Envelope, error) {
	n.mu.Lock()
	n.runCount++
	n.mu.Unlock()
...
func buildTestGraph(nodeIDs ...string) graph.Graph {
	builder := graph.NewGraphBuilder("test-graph")
...
func TestCallbackStepController_BasicStepThrough(t *testing.T) {
	...
	g := buildTestGraph("node1", "node2", "node3")
	runtime := NewRuntime()
	...
	result, err := runtime.Run(context.Background(), g, env, opts)
	if err != nil {
...
func TestCallbackStepController_PauseAfterNode(t *testing.T) {
	...
	_, err := runtime.Run(context.Background(), g, env, opts)
	if err != nil {
		t.Fatalf("Run failed: %v", err)
	}
...
func TestChannelStepController_Interactive(t *testing.T) {
	...
	go func() {
		result, runErr = runtime.Run(context.Background(), g, env, opts)
		close(done)
	}()
...
func TestChannelStepController_Breakpoints(t *testing.T) {
	...
	go func() {
		_, _ = runtime.Run(context.Background(), g, env, opts)
		close(done)
	}()
...
func TestStepController_ContextCancellation(t *testing.T) {
	...
	_, err := runtime.Run(ctx, g, env, opts)
	if err == nil {
