...
func createEnvelopeSnapshot(env *core.Envelope) *EnvelopeSnapshot {
	if env == nil {
		return nil
	}
	...
	snapshot := &EnvelopeSnapshot{
		...
		Artifacts: make([]core.Artifact, len(env.Artifacts)),
		Messages:  make([]core.Message, len(env.Messages)),
		Errors:    make([]core.NodeError, len(env.Errors)),
		Trace:     env.Trace,
	}
...
func createGraphSnapshot(g graph.Graph, currentNode string) GraphSnapshot {
	nodes := g.Nodes()
