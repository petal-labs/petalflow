...
func TestRuntime_Run_NodeError(t *testing.T) {
	...
	g.AddNode(core.NewFuncNode("fail", func(ctx context.Context, env *core.Envelope) (*core.Envelope, error) {
...
func TestRuntime_Run_ContinueOnError(t *testing.T) {
	g := graph.NewGraph("continue-test")
	g.AddNode(core.NewFuncNode("fail", func(ctx context.Context, env *core.Envelope) (*core.Envelope, error) {
...
func TestRuntime_Run_EventNodeFailed(t *testing.T) {
	...
	rt.Run(context.Background(), g, core.NewEnvelope(), opts)
...
func TestRuntime_Run_ExecutionOrder(t *testing.T) {
	...
	g := graph.NewGraph("order-test")
	for _, id := range []string{"a", "b", "c"} {
		nodeID := id
		g.AddNode(core.NewFuncNode(nodeID, func(ctx context.Context, env *core.Envelope) (*core.Envelope, error) {
...
func TestRuntime_Run_RouterNode_SingleTarget(t *testing.T) {
	...
	g := graph.NewGraph("router-test")

	// Add the router
	router := nodes.NewRuleRouter("router", nodes.RuleRouterConfig{
		Rules: []nodes.RouteRule{
			{
				Conditions: []nodes.RouteCondition{
					{VarPath: "route", Op: nodes.OpEquals, Value: "a"},
				},
				Target: "handler-a",
				Reason: "Route to A",
			},
	...
	// Add handlers
	g.AddNode(core.NewFuncNode("handler-a", func(ctx context.Context, env *core.Envelope) (*core.Envelope, error) {
		executed["handler-a"] = true
		return env, nil
	}))
	g.AddNode(core.NewFuncNode("handler-b", func(ctx context.Context, env *core.Envelope) (*core.Envelope, error) {
...
func TestRuntime_Run_RouterNode_DefaultTarget(t *testing.T) {
	...
	g := graph.NewGraph("router-default")

	router := nodes.NewRuleRouter("router", nodes.RuleRouterConfig{
		Rules: []nodes.RouteRule{
			{
				Conditions: []nodes.RouteCondition{
					{VarPath: "route", Op: nodes.OpEquals, Value: "special"},
				},
	...
	g.AddNode(core.NewFuncNode("special-handler", func(ctx context.Context, env *core.Envelope) (*core.Envelope, error) {
		executed["special-handler"] = true
		return env, nil
	}))
	g.AddNode(core.NewFuncNode("default-handler", func(ctx context.Context, env *core.Envelope) (*core.Envelope, error) {
...
func TestRuntime_Run_RouterNode_MultipleTargets(t *testing.T) {
	...
	g := graph.NewGraph("router-multi")

	router := nodes.NewRuleRouter("router", nodes.RuleRouterConfig{
		Rules: []nodes.RouteRule{
			{
				Conditions: []nodes.RouteCondition{
					{VarPath: "notify_email", Op: nodes.OpExists},
				},
				Target: "email-handler",
			},
			{
				Conditions: []nodes.RouteCondition{
					{VarPath: "notify_sms", Op: nodes.OpExists},
				},
				Target: "sms-handler",
			},
		},
	...
	g.AddNode(core.NewFuncNode("email-handler", func(ctx context.Context, env *core.Envelope) (*core.Envelope, error) {
		executed["email-handler"] = true
		return env, nil
	}))
	g.AddNode(core.NewFuncNode("sms-handler", func(ctx context.Context, env *core.Envelope) (*core.Envelope, error) {
...
func TestRuntime_Run_RouterNode_EventEmitted(t *testing.T) {
	...
	g.AddNode(router)
	g.AddNode(core.NewNoopNode("next"))
...
func TestRuntime_Run_RouterNode_ChainedRouters(t *testing.T) {
	...
	g := graph.NewGraph("chained-routers")

	router1 := nodes.NewRuleRouter("router1", nodes.RuleRouterConfig{
		Rules: []nodes.RouteRule{
	...
	g.AddNode(core.NewFuncNode("middle", func(ctx context.Context, env *core.Envelope) (*core.Envelope, error) {
		executed = append(executed, "middle")
		env.SetVar("second_route", "final")
		return env, nil
	}))

	router2 := nodes.NewRuleRouter("router2", nodes.RuleRouterConfig{
		Rules: []nodes.RouteRule{
			{
				Conditions: []nodes.RouteCondition{
					{VarPath: "second_route", Op: nodes.OpEquals, Value: "final"},
				},
				Target: "final",
			},
		},
	...
	g.AddNode(core.NewFuncNode("alternate", func(ctx context.Context, env *core.Envelope) (*core.Envelope, error) {
...
func TestRuntime_Run_RouterNode_NoTargetsMatched(t *testing.T) {
	...
	g := graph.NewGraph("router-no-match")

	router := nodes.NewRuleRouter("router", nodes.RuleRouterConfig{
		Rules: []nodes.RouteRule{
			{
				Conditions: []nodes.RouteCondition{
					{VarPath: "impossible", Op: nodes.OpEquals, Value: "never"},
				},
				Target: "handler",
			},
		},
	...
	g.AddNode(core.NewFuncNode("handler", func(ctx context.Context, env *core.Envelope) (*core.Envelope, error) {
...
func TestRuntime_Run_Concurrent_SimpleFanOut(t *testing.T) {
	...
	g := graph.NewGraph("concurrent-fanout")
...
func TestRuntime_Run_Concurrent_FanOutMerge(t *testing.T) {
	...
	g := graph.NewGraph("concurrent-merge")
	g.AddNode(core.NewFuncNode("start", func(ctx context.Context, env *core.Envelope) (*core.Envelope, error) {
		mu.Lock()
	...
	merger := nodes.NewMergeNode("merge", nodes.MergeNodeConfig{
		Strategy: nodes.NewJSONMergeStrategy(nodes.JSONMergeConfig{}),
	})
	g.AddNode(merger)
...
func TestRuntime_Run_Concurrent_MergeNodeWithStrategy(t *testing.T) {
	g := graph.NewGraph("merge-strategy")
	g.AddNode(core.NewNoopNode("start"))
	g.AddNode(core.NewFuncNode("branch-a", func(ctx context.Context, env *core.Envelope) (*core.Envelope, error) {
	...
	merger := nodes.NewMergeNode("merge", nodes.MergeNodeConfig{
		Strategy: nodes.NewBestScoreMergeStrategy(nodes.BestScoreMergeConfig{
			ScoreVar:       "score",
			HigherIsBetter: true,
		}),
	})
...
func TestRuntime_Run_Concurrent_ErrorHandling(t *testing.T) {
	...
	g.AddNode(core.NewNoopNode("branch-b"))
...
func TestRuntime_Run_Concurrent_ErrorContinue(t *testing.T) {
	...
	g := graph.NewGraph("concurrent-continue")
	g.AddNode(core.NewNoopNode("start"))
	g.AddNode(core.NewFuncNode("branch-a", func(ctx context.Context, env *core.Envelope) (*core.Envelope, error) {
		return nil, errors.New("branch-a failed")
	}))
...
func TestRuntime_Run_Concurrent_ContextCancellation(t *testing.T) {
	g := graph.NewGraph("concurrent-cancel")
	g.AddNode(core.NewFuncNode("start", func(ctx context.Context, env *core.Envelope) (*core.Envelope, error) {
		return env, nil
	}))
	g.AddNode(core.NewFuncNode("slow", func(ctx context.Context, env *core.Envelope) (*core.Envelope, error) {
...
func TestRuntime_Run_Concurrent_DiamondPattern(t *testing.T) {
	...
	g.AddNode(core.NewFuncNode("end", func(ctx context.Context, env *core.Envelope) (*core.Envelope, error) {
		mu.Lock()
...
func TestRuntime_Run_GateNode_Pass(t *testing.T) {
	...
	g.AddNode(nodes.NewGateNode("gate", nodes.GateNodeConfig{
...
func TestRuntime_Run_GateNode_Redirect(t *testing.T) {
	...
	g := graph.NewGraph("gate-redirect")
	...
	g.AddNode(nodes.NewGateNode("gate", nodes.GateNodeConfig{
		ConditionVar:   "authorized",
		OnFail:         nodes.GateActionRedirect,
		RedirectNodeID: "error_handler",
	}))

	g.AddNode(core.NewFuncNode("success", func(ctx context.Context, env *core.Envelope) (*core.Envelope, error) {
		env.SetVar("path", "success")
		return env, nil
	}))

	g.AddNode(core.NewFuncNode("error_handler", func(ctx context.Context, env *core.Envelope) (*core.Envelope, error) {
...
func TestRuntime_Run_MapNode(t *testing.T) {
	...
	g.AddNode(nodes.NewMapNode("mapper", nodes.MapNodeConfig{
		InputVar: "numbers",
