...
func (b *GraphBuilder) AddNode(node core.Node) *GraphBuilder {
	if node == nil {
...
func (b *GraphBuilder) Edge(node core.Node) *GraphBuilder {
	if node == nil {
		b.errors = append(b.errors, fmt.Errorf("cannot add nil node"))
		return b
	}
...
func (b *GraphBuilder) FanOut(nodes ...core.Node) *GraphBuilder {
	if len(nodes) == 0 {
...
func (b *GraphBuilder) Merge(mergeNode core.MergeCapable) *GraphBuilder {
	if mergeNode == nil {
	...
	if mergeNode.ExpectedInputs() == 0 {
		mergeNode.SetExpectedInputs(len(b.current))
	}
...
// Does not change the current node.
func (b *GraphBuilder) WithNodes(nodes ...core.Node) *GraphBuilder {
	for _, node := range nodes {
		if node == nil {
			b.errors = append(b.errors, fmt.Errorf("cannot add nil node"))
			continue
...
func (b *GraphBuilder) Conditional(router core.RouterNode, targets ...core.Node) *GraphBuilder {
	if router == nil {
