...
type FilterOp struct {
	...
	CustomFunc func(item any, index int, env *core.Envelope) (keep bool, err error)
}
...
func NewFilterNode(id string, config FilterNodeConfig) *FilterNode {
	...
	return &FilterNode{
		BaseNode: core.NewBaseNode(id, core.NodeKindFilter),
		config:   config,
	}
}
...
func (n *FilterNode) Run(ctx context.Context, env *core.Envelope) (*core.Envelope, error) {
	result := env.Clone()
...
func (n *FilterNode) applyFilter(ctx context.Context, env *core.Envelope, items []any, op FilterOp) ([]any, error) {
	switch op.Type {
	case FilterOpTopN:
...
func (n *FilterNode) filterCustom(ctx context.Context, env *core.Envelope, items []any, op FilterOp) ([]any, error) {
...
// getArtifactField gets a field value from a core.Artifact.
func getArtifactField(a core.Artifact, field string) any {
	switch field {
	case "ID", "id":
		return a.ID
	case "Type", "type":
...
// getMessageField gets a field value from a core.Message.
func getMessageField(m core.Message, field string) any {
	switch field {
	case "Role", "role":
		return m.Role
...
var _ core.Node = (*FilterNode)(nil)
