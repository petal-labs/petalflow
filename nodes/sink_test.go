...
func TestSinkNode_IncludeArtifacts(t *testing.T) {
	...
	env := core.NewEnvelope()
	env.AppendArtifact(core.Artifact{ID: "art1", Type: "text", Text: "content"})
...
func TestSinkNode_IncludeMessages(t *testing.T) {
	...
	env := core.NewEnvelope()
	env.AppendMessage(core.Message{Role: "user", Content: "hello"})
...
func TestSinkNode_Kind(t *testing.T) {
	...
	if node.Kind() != core.NodeKindSink {
		t.Errorf("Kind = %q, want %q", node.Kind(), core.NodeKindSink)
	}
}
