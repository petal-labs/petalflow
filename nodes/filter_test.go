...
func TestNewFilterNode(t *testing.T) {
	t.Run("defaults", func(t *testing.T) {
		...
		if node.Kind() != core.NodeKindFilter {
			t.Errorf("expected kind %v, got %v", core.NodeKindFilter, node.Kind())
		}
	...
	t.Run("custom filter function", func(t *testing.T) {
		node := NewFilterNode("custom", FilterNodeConfig{
			...
			Filters: []FilterOp{
				{
					Type: FilterOpCustom,
					CustomFunc: func(item any, index int, env *core.Envelope) (bool, error) {
						num, ok := item.(int)
						if !ok {
							return false, nil
						}
