...
func TestCacheNode_IncludeArtifacts(t *testing.T) {
	...
	env1 := core.NewEnvelope()
	env1.AppendArtifact(core.Artifact{ID: "art1", Type: "text", Text: "hello"})
	node.Run(context.Background(), env1)

	// Second call with same artifact - should hit
	env2 := core.NewEnvelope()
	env2.AppendArtifact(core.Artifact{ID: "art1", Type: "text", Text: "hello"})
	result, _ := node.Run(context.Background(), env2)
	...
	env3 := core.NewEnvelope()
	env3.AppendArtifact(core.Artifact{ID: "art2", Type: "text", Text: "world"})
	result, _ = node.Run(context.Background(), env3)
...
func TestCacheNode_Kind(t *testing.T) {
	...
	if node.Kind() != core.NodeKindCache {
		t.Errorf("Kind = %q, want %q", node.Kind(), core.NodeKindCache)
	}
}
...
func TestMemoryCacheStore_Size(t *testing.T) {
	...
	store.Set(ctx, "key1", core.NewEnvelope(), 0)
	store.Set(ctx, "key2", core.NewEnvelope(), 0)
...
func TestMemoryCacheStore_Keys(t *testing.T) {
	...
	store.Set(ctx, "charlie", core.NewEnvelope(), 0)
	store.Set(ctx, "alpha", core.NewEnvelope(), 0)
	store.Set(ctx, "bravo", core.NewEnvelope(), 0)
...
func TestMemoryCacheStore_Prune(t *testing.T) {
	...
	store.Set(ctx, "expire1", core.NewEnvelope(), 10*time.Millisecond)
	store.Set(ctx, "expire2", core.NewEnvelope(), 10*time.Millisecond)
	store.Set(ctx, "persist", core.NewEnvelope(), 0) // No TTL
...
func TestMemoryCacheStore_ContextCancellation(t *testing.T) {
	...
	err = store.Set(ctx, "key", core.NewEnvelope(), 0)
	if !errors.Is(err, context.Canceled) {
