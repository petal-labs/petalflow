...
type MergeStrategy interface {
	...
	Merge(ctx context.Context, inputs []*core.Envelope) (*core.Envelope, error)
}
...
func NewMergeNode(id string, config MergeNodeConfig) *MergeNode {
	...
	return &MergeNode{
		BaseNode: core.NewBaseNode(id, core.NodeKindMerge),
		config:   config,
	}
}
...
func (n *MergeNode) Run(ctx context.Context, env *core.Envelope) (*core.Envelope, error) {
...
func (n *MergeNode) MergeInputs(ctx context.Context, inputs []*core.Envelope) (*core.Envelope, error) {
	if len(inputs) == 0 {
...
// SetExpectedInputs sets the number of inputs this merge node expects.
// This is called by the graph builder and runtime to configure merge behavior.
func (n *MergeNode) SetExpectedInputs(count int) {
...
func (s *JSONMergeStrategy) Merge(ctx context.Context, inputs []*core.Envelope) (*core.Envelope, error) {
	if len(inputs) == 0 {
		return core.NewEnvelope(), nil
	}
...
func (s *ConcatMergeStrategy) Merge(ctx context.Context, inputs []*core.Envelope) (*core.Envelope, error) {
	if len(inputs) == 0 {
		return core.NewEnvelope(), nil
	}
...
func (s *BestScoreMergeStrategy) Merge(ctx context.Context, inputs []*core.Envelope) (*core.Envelope, error) {
	if len(inputs) == 0 {
		return core.NewEnvelope(), nil
	}

	var bestEnv *core.Envelope
...
type FuncMergeStrategy struct {
	name string
	fn   func(ctx context.Context, inputs []*core.Envelope) (*core.Envelope, error)
}

// NewFuncMergeStrategy creates a custom merge strategy from a function.
func NewFuncMergeStrategy(name string, fn func(ctx context.Context, inputs []*core.Envelope) (*core.Envelope, error)) *FuncMergeStrategy {
...
func (s *FuncMergeStrategy) Merge(ctx context.Context, inputs []*core.Envelope) (*core.Envelope, error) {
	if s.fn == nil {
		if len(inputs) > 0 {
...
func (s *AllMergeStrategy) Merge(ctx context.Context, inputs []*core.Envelope) (*core.Envelope, error) {
...
var (
	_ core.Node         = (*MergeNode)(nil)
	_ core.MergeCapable = (*MergeNode)(nil)
