...
type CacheStore interface {
	...
	Get(ctx context.Context, key string) (*core.Envelope, bool, error)

	// Set stores an envelope with the given key.
	// TTL of 0 means no expiration.
	Set(ctx context.Context, key string, env *core.Envelope, ttl time.Duration) error
...
func NewCacheNode(id string, config CacheNodeConfig) *CacheNode {
	...
	return &CacheNode{
		BaseNode: core.NewBaseNode(id, core.NodeKindCache),
		config:   config,
	}
}
...
func (n *CacheNode) Run(ctx context.Context, env *core.Envelope) (*core.Envelope, error) {
	// Check context
...
func (n *CacheNode) generateCacheKey(env *core.Envelope) (string, error) {
...
func (n *CacheNode) renderCacheKeyTemplate(env *core.Envelope) (string, error) {
...
func (n *CacheNode) computeCacheKeyHash(env *core.Envelope) (string, error) {
	// Build data to hash
...
// Get retrieves a cached envelope by key.
func (s *MemoryCacheStore) Get(ctx context.Context, key string) (*core.Envelope, bool, error) {
	if err := ctx.Err(); err != nil {
...
// Set stores an envelope with the given key.
func (s *MemoryCacheStore) Set(ctx context.Context, key string, env *core.Envelope, ttl time.Duration) error {
	if err := ctx.Err(); err != nil {
		return err
	}
...
func NewMockNode(id string, returnValue any) *MockNode {
	return &MockNode{
		BaseNode:    core.NewBaseNode(id, core.NodeKindNoop),
		ReturnValue: returnValue,
	}
}

// Run executes the mock node.
func (n *MockNode) Run(ctx context.Context, env *core.Envelope) (*core.Envelope, error) {
	n.mu.Lock()
	n.CallCount++
	n.mu.Unlock()
